...
type PushTemplate = 'text-only' | 'text-sms' | 'text-browser';

interface Send {
  id: string;
  template: PushTemplate;
  pushText: string;
  // Template 2 (text-sms) específicos
  smsText?: string;
  shortCode?: string;
  // Template 3 (text-browser) específico
  targetUrl?: string;
...
interface NewPushCampaignFormProps {
  onClose: () => void;
  onSubmit: (data: {
    name: string;
    description: string;
...
export default function NewPushCampaignForm({ onClose, onSubmit }: NewPushCampaignFormProps) {
  ...
  const [template, setTemplate] = useState<PushTemplate>('text-only');
  ...
  const templates = [
    { 
      value: 'text-only' as PushTemplate, 
      label: 'Solo Texto', 
      description: 'Mensaje informativo SAT PUSH sin interacción',
      icon: '📝'
    },
    { 
      value: 'text-sms' as PushTemplate, 
      label: 'Texto + SMS', 
      description: 'SAT PUSH con botón que emula envío de SMS',
      icon: '💬'
    },
    { 
      value: 'text-browser' as PushTemplate, 
      label: 'Texto + Navegador', 
      description: 'SAT PUSH con botón que abre URL',
      icon: '🌐'
    }
  ];
  ...
  const steps = [
    ...
    { number: 2, title: 'Crear Envíos', description: 'Configurar envíos SAT PUSH' },
  ...
  const handleEditSend = (index: number) => {
    ...
    setIsCreatingOrEditingSend(true);
    setTemplate(send.template);
    setPushText(send.pushText);
    setSmsText(send.smsText || '');
    setShortCode(send.shortCode || '');
    setTargetUrl(send.targetUrl || '');
  ...
  const handleSaveSend = () => {
    ...
    if (!pushText) {
      alert('Por favor completa el texto del mensaje SAT PUSH');
      return;
    }

    // Validaciones específicas por template
    if (template === 'text-sms') {
      if (!smsText || !shortCode) {
        alert('Por favor completa el texto del SMS y el código corto');
        return;
      }
    }

    if (template === 'text-browser') {
      if (!targetUrl) {
        alert('Por favor completa la URL de destino');
        return;
      }
    }
    ...
    const newSend: Send = {
      ...
      template,
      pushText,
      smsText: template === 'text-sms' ? smsText : undefined,
      shortCode: template === 'text-sms' ? shortCode : undefined,
      targetUrl: template === 'text-browser' ? targetUrl : undefined,
      segmentationType,
  ...
  const resetSendForm = () => {
    ...
    setSelectedProducts([]);
    setSendSchedule({ id: '1', date: '', time: '' });
    setEditingSendIndex(null);
  };

  const handleScheduleChange = (field: keyof SendSchedule, value: string | File | null) => {
  ...
  const getTemplateLabel = (templateValue: PushTemplate) => {
    return templates.find(t => t.value === templateValue)?.label || templateValue;
  };
    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 overflow-y-auto p-4">
      <div className="bg-white rounded-lg shadow-xl w-full max-w-5xl max-h-[90vh] overflow-hidden flex flex-col">
        ...
        <div className="bg-white border-b border-gray-200 px-6 py-4 flex items-center justify-between">
          <div>
            <h2 className="text-xl font-semibold text-gray-900">Nueva Campaña SAT PUSH</h2>
          ...
          <div className="p-6">
              ...
              <div className="space-y-4 animate-fadeIn">
                <div className="p-4 bg-blue-50 border border-blue-200 rounded-lg">
                  ...
                  <p className="text-sm text-blue-800">
                    Define la información general de tu campaña SAT PUSH. Después podrás crear múltiples envíos con diferentes templates.
                  </p>
                </div>
                ...
                <div>
                  ...
                  <input
                    ...
                    placeholder="Ej: Black Friday 2025 SAT PUSH"
                  />
                </div>
                ...
                <div>
                  ...
                  <textarea
                    ...
                    rows={4}
                    className="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent"
                    placeholder="Describe el objetivo general de la campaña SAT PUSH, su propósito y alcance esperado..."
                  />
                </div>
            ...
            {currentStep === 2 && (
              <div className="space-y-6 animate-fadeIn">
                <div className="p-4 bg-blue-50 border border-blue-200 rounded-lg">
                  <h3 className="text-sm font-semibold text-blue-900 mb-2">Paso 2: Crear Envíos SAT PUSH</h3>
                  <p className="text-sm text-blue-800">
                    Crea uno o varios envíos para la campaña "{campaignName}". Cada envío puede usar un template diferente y tener su propia segmentación.
                  </p>
                </div>
                                ...
                                <div className="flex-1">
                                  <div className="flex items-center gap-3 mb-3">
                                    ...
                                    <span className="inline-flex items-center px-3 py-1 rounded-full text-sm font-medium bg-purple-100 text-purple-800">
                                      {templates.find(t => t.value === send.template)?.icon} {getTemplateLabel(send.template)}
                                    </span>
                                  </div>
                                  
                                  <div className="space-y-2 text-sm">
                                    <div>
                                      <span className="font-medium text-gray-700">Mensaje SAT PUSH:</span>
                                      <p className="mt-1 font-mono text-xs bg-gray-50 p-3 rounded border border-gray-200 text-gray-900">{send.pushText}</p>
                                    </div>
                                    
                                    {send.template === 'text-sms' && (
                                      <div className="pl-4 border-l-2 border-purple-200">
                                        <p className="text-xs text-purple-700 font-medium mb-1">Configuración SMS:</p>
                                        <div className="text-xs space-y-1">
                                          <div><span className="font-medium">Short Code:</span> {send.shortCode}</div>
                                          <div><span className="font-medium">Texto SMS:</span> {send.smsText}</div>
                                        </div>
                                      </div>
                                    )}
                                    
                                    {send.template === 'text-browser' && (
                                      <div className="pl-4 border-l-2 border-green-200">
                                        <p className="text-xs text-green-700 font-medium mb-1">URL de Destino:</p>
                                        <div className="text-xs">
                                          <a href={send.targetUrl} target="_blank" rel="noopener noreferrer" className="text-blue-600 hover:underline">
                                            {send.targetUrl}
                                          </a>
                                        </div>
                                      </div>
                ...
                {isCreatingOrEditingSend && (
                  <div className="border-2 border-blue-300 rounded-lg p-6 bg-blue-50/30">
                    <h4 className="text-lg font-semibold text-gray-900 mb-4">
                      {editingSendIndex !== null ? `Editando Envío #${editingSendIndex + 1}` : 'Nuevo Envío SAT PUSH'}
                    </h4>
                    ...
                    <div className="space-y-6">
                          ...
                          {templates.map(tmpl => (
                            ...
                            >
                              <input
                                ...
                                onChange={(e) => setTemplate(e.target.value as PushTemplate)}
                                className="sr-only"
                              />
                              <div className="text-center">
                                <div className="text-3xl mb-2">{tmpl.icon}</div>
                                <p className="font-medium text-gray-900 text-sm mb-1">{tmpl.label}</p>
                      ...
                      <div className="pt-4 border-t border-gray-200">
                        ...
                        <textarea
                          ...
                          placeholder="Escribe el texto que aparecerá en el mensaje SAT PUSH..."
                        />
                        <p className="mt-1 text-xs text-gray-500">
                          {pushText.length}/160 caracteres
                        </p>
                      </div>

                      {/* Campos específicos para Template 2 (text-sms) */}
                      {template === 'text-sms' && (
                        <div className="p-4 bg-purple-50 border border-purple-200 rounded-lg space-y-4">
                          ...
                          <div>
                            ...
                            <p className="mt-1 text-xs text-gray-500">
                              Código corto al que se enviará el SMS emulado
                            </p>
                          </div>

                          <div>
                            <label className="block text-sm font-medium text-gray-700 mb-2">
                              Texto del SMS a Emular *
                            </label>
                            <textarea
                              required
                              value={smsText}
                              ...
                              maxLength={160}
                              className="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent font-mono text-sm bg-white"
                              placeholder="Ej: SI para aceptar la oferta"
                            />
                            <p className="mt-1 text-xs text-gray-500">
                              {smsText.length}/160 caracteres - Este texto se enviará automáticamente cuando el usuario acepte
                            </p>
                          </div>
                        </div>
                      )}

                      {/* Campos específicos para Template 3 (text-browser) */}
                      {template === 'text-browser' && (
                        <div className="p-4 bg-green-50 border border-green-200 rounded-lg">
                          <h5 className="text-sm font-semibold text-green-900 flex items-center gap-2 mb-4">
                          ...
                          <div>
                            ...
                            />
                            <p className="mt-1 text-xs text-gray-500">
                              URL completa a la que será redirigido el usuario tras aceptar el mensaje
                            </p>
                          </div>
                        </div>
                      )}
                        ...
                        <div className="space-y-3">
                          ...
                          <label className={`flex items-start gap-3 p-4 border-2 rounded-lg cursor-pointer transition-all ${
                            ...
                            <div className="flex-1">
                              ...
                              <p className="text-sm text-gray-600 mt-1">
                                Cargarás archivo CSV en la programación
                              </p>
                            </div>
                          </label>
                        </div>
            ...
            {currentStep === 3 && (
              <div className="space-y-6 animate-fadeIn">
                <div className="p-4 bg-blue-50 border border-blue-200 rounded-lg">
                  ...
                  <p className="text-sm text-blue-800">
                    Revisa toda la configuración de la campaña SAT PUSH y sus envíos antes de confirmar.
                  </p>
                </div>
                  ...
                  <div className="pl-8 space-y-4">
                    {sends.map((send, index) => (
                      <div key={send.id} className="border border-gray-200 rounded-lg p-4 bg-gray-50">
                        ...
                        <div className="space-y-2 text-sm">
                          <div className="py-1">
                            <span className="text-gray-600 block mb-1">Mensaje SAT PUSH:</span>
                            <div className="bg-white p-3 rounded border border-gray-200">
                              <span className="text-sm font-mono text-gray-900">{send.pushText}</span>
                            </div>
                          </div>

                          {send.template === 'text-sms' && (
                            <div className="py-1 pl-4 border-l-2 border-purple-200">
                              <span className="text-purple-700 font-medium text-xs block mb-1">Configuración SMS:</span>
                              <div className="text-xs space-y-1">
                                <div><span className="font-medium">Short Code:</span> {send.shortCode}</div>
                                <div><span className="font-medium">Texto SMS:</span> {send.smsText}</div>
                              </div>
                          ...
                          {send.template === 'text-browser' && (
                            <div className="py-1 pl-4 border-l-2 border-green-200">
                              <span className="text-green-700 font-medium text-xs block mb-1">URL de Destino:</span>
                              <div className="text-xs">
                                <a href={send.targetUrl} target="_blank" rel="noopener noreferrer" className="text-blue-600 hover:underline">
                                  {send.targetUrl}
                                </a>
                ...
                <div className="p-4 bg-yellow-50 border border-yellow-200 rounded-lg">
                  ...
                  <p className="text-sm text-yellow-800">
                    Al confirmar, se creará la campaña SAT PUSH con {sends.length} envío(s) configurado(s). Cada envío se ejecutará según su programación establecida.
                  </p>
                </div>
              </div>
            )}
